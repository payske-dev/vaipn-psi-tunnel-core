...
public class VaipnTunnel {
    ...
    // Only one VaipnVpn instance may exist at a time, as the underlying
    // psi.Psi and tun2socks implementations each contain global state.
    private static VaipnTunnel mVaipnTunnel;

    public static synchronized VaipnTunnel newVaipnTunnel(HostService hostService) {
        return newVaipnTunnelImpl(hostService, true);
    }

    // The two argument override in case the host app wants to take control over calling routeThroughTunnel()
    public static synchronized VaipnTunnel newVaipnTunnel(HostService hostService, boolean shouldRouteThroughTunnelAutomatically) {
        return newVaipnTunnelImpl(hostService, shouldRouteThroughTunnelAutomatically);
    }

    private static VaipnTunnel newVaipnTunnelImpl(HostService hostService, boolean shouldRouteThroughTunnelAutomatically) {
        if (mVaipnTunnel != null) {
            mVaipnTunnel.stop();
        }
        // Load the native go code embedded in psi.aar
        hostService.loadLibrary("gojni");
        mVaipnTunnel = new VaipnTunnel(hostService, shouldRouteThroughTunnelAutomatically);
        return mVaipnTunnel;
    }
    ...
    private static File defaultDataRootDirectory(Context context) {
        return context.getFileStreamPath("ca.vaipnpsi.VaipnTunnel.tunnel-core");
    }

    private VaipnTunnel(HostService hostService, boolean shouldRouteThroughTunnelAutomatically) {
    ...
    // VaipnTunnel.newVaipnTunnel(HostService hostservice, shouldRouteThroughTunnelAutomatically = false)
    ...
    // did not request a manual control over this functionality, see VaipnTunnel.newVaipnTunnel
    public void routeThroughTunnel() {
        ...
        try {
            ...
            // https://bitbucket.org/psiphon/psiphon-circumvention-system/src/1dc5e4257dca99790109f3bf374e8ab3a0ead4d7/Android/VaipnAndroidLibrary/src/com/psiphon3/psiphonlibrary/TunnelCore.java?at=default#cl-779
        } catch (IOException e) {
            ...
            workQueue.execute(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        String psiphonConfig = buildVaipnConfig(context, logger, feedbackConfigJson,
    ...
    private class VaipnProviderShim implements PsiphonProvider {
    ...
    private String getDNSServers(Context context, HostLogger logger) {
        ...
        try {
            ...
            // Alternate DNS servers will be provided by vaipn-psi-tunnel-core
            // config or tactics.
        }
        return servers;
    }
    ...
    private String loadVaipnConfig(Context context)
        ...
        return buildVaipnConfig(context, mHostService, mHostService.getVaipnConfig(),
                mClientPlatformPrefix.get(), mClientPlatformSuffix.get(), isVpnMode(),
                mLocalSocksProxyPort.get());
    }

    private static String buildVaipnConfig(Context context, HostLogger logger, String psiphonConfig,
                                             String clientPlatformPrefix, String clientPlatformSuffix,
    ...
    public static void logTun2Socks(String level, String channel, String msg) {
        ...
        mVaipnTunnel.mHostService.onDiagnosticMessage(logMsg);
    }
            ...
            networkCallback = new ConnectivityManager.NetworkCallback() {
                ...
                private void setCurrentActiveNetworkAndProperties(Network network) {
                    ...
                    if (network == null) {
                        ...
                        mVaipnTunnel.mActiveNetworkType.set("NONE");
                        mVaipnTunnel.mActiveNetworkDNSServers.set("");
                        mVaipnTunnel.mHostService.onDiagnosticMessage("NetworkMonitor: clear current active network");
                        ...
                        mVaipnTunnel.mActiveNetworkType.set(networkType);
                        ...
                        // Use the workaround, comma-delimited format required for gobind.
                        mVaipnTunnel.mActiveNetworkDNSServers.set(TextUtils.join(",", servers));
                        ...
                        mVaipnTunnel.mHostService.onDiagnosticMessage(message);
                    }
