...
# build-psiphon-framework.sh builds a VaipnTunnel.xcframework bundle
...
TUNNEL_CORE_SRC_DIR=${GOPATH}/src/github.com/payske-dev/vaipn-psi-tunnel-core
...
ln -s "${BASE_DIR}/../.." "${GOPATH}/src/github.com/payske-dev/vaipn-psi-tunnel-core"
if [[ $? != 0 ]]; then
  echo "ln -s ../.. ${GOPATH}/src/github.com/payske-dev/vaipn-psi-tunnel-core"
  exit 1
fi
...
function gomobile_build_for_platform() {
  ...
  -tags="${BUILD_TAGS}" \
  -ldflags="${LDFLAGS}" \
  -o "${GOBIND_OUT}" github.com/payske-dev/vaipn-psi-tunnel-core/MobileLibrary/psi
...
cp -R "${GOPATH}/src/github.com/payske-dev/vaipn-psi-tunnel-core/MobileLibrary/go-mobile" "${GOPATH}/src/golang.org/x/mobile"
...
BUILDINFOFILE="${BASE_DIR}/vaipn-psi-tunnel-core_buildinfo.txt"
